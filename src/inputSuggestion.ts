 ...
 */
export class InputSuggestionService {
    private _disposable: vscode.Disposable | undefined;
    private _statusBarItem: vscode.StatusBarItem;
    private _suggestionPanel: vscode.WebviewView | undefined;
    private _promptManager: PromptManager;
    private _lastInput: string = '';
    private _suggestions: Prompt[] = [];
     ...
     * 文本变化时的处理
     */
    private async onTextChanged(e: vscode.TextDocumentChangeEvent) {
     ...
     * 根据输入文本更新建议
     */
    private async updateSuggestionsBasedOnInput(input: string) {
     ...
     * 匹配提示词
     */
    private matchPrompts(input: string, prompts: Prompt[]): Prompt[] {
     ...
     * 显示状态栏项
     */
    private showStatusBarItem(count: number) {
     ...
     * 插入提示词（用于AI对话 - 复制到剪贴板）
     */
    private async insertPrompt(prompt: Prompt) {
        // 复制到剪贴板供AI对话使用
     ...
     * 显示建议下拉列表（在输入框附近）
     */
    async showInlineSuggestions(): Promise<Prompt | undefined> {
        const suggestions = this.getSuggestions();
        ...
        quickPick.items = suggestions.map(s => ({
            ...
            prompt: s
        } as vscode.QuickPickItem & { prompt: Prompt }));

        return new Promise((resolve) => {
            quickPick.onDidAccept(() => {
                const selected = quickPick.selectedItems[0] as vscode.QuickPickItem & { prompt: Prompt };
                if (selected && selected.prompt) {
