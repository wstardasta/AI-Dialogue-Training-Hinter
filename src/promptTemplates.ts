...
export class PromptTemplateManager {
     ...
     * 获取所有系统模板
     */
    static getSystemTemplates(): PromptTemplate[] {
     ...
     * 根据语言和类别获取推荐的模板
     */
    static getRecommendedTemplates(language?: string, category?: string): PromptTemplate[] {
        let templates = this.systemTemplates;

        if (language) {
            templates = templates.filter(t => 
                !t.language || t.language.length === 0 || t.language.includes(language)
            );
        }
     ...
     * 将模板转换为提示词
     */
    static templateToPrompt(template: PromptTemplate, context: any): Prompt {
