...
class CodeAnalyzer {
     ...
     * 获取当前代码上下文
     */
    static async getCurrentCodeContext() {
     ...
     * 基于代码上下文生成提示词
     */
    static async generatePromptFromCode(context, template) {
     ...
     * 生成默认提示词
     */
    static generateDefaultPrompt(context) {
     ...
     * 替换模板变量
     */
    static replaceTemplateVariables(template, context, languageInfo, projectInfo) {
        let result = template;
        // 替换代码占位符
     ...
     * 提取代码的关键信息用于匹配提示词
     */
    static extractKeywords(context) {
        const keywords = [];
        // 从代码中提取关键词
