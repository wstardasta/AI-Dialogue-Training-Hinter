...
class LanguageDetector {
     ...
     * 根据语言ID获取分类
     */
    static getCategoryFromLanguage(languageId) {
        const categoryMap = {
            'javascript': '代码生成',
            'typescript': '代码生成',
            'python': '代码生成',
     ...
     * 检测包管理器
     */
    static async detectPackageManager(rootPath) {
        const fs = require('fs');
     ...
     * 根据语言和项目类型推荐提示词类别
     */
    static recommendCategories(languageInfo, projectInfo) {
        const categories = [];
        if (languageInfo) {
