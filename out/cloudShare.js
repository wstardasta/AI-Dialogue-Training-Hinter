...
exports.CloudShareManager = void 0;
...
class CloudShareManager {
     ...
     * 上传提示词到云端
     */
    static async uploadPrompt(prompt) {
        if (this.USE_MOCK) {
            // 模拟上传，实际应该调用真实API
     ...
     * 搜索云端提示词
     */
    static async searchPrompts(keyword) {
        const allPrompts = await this.downloadPrompts();
        const lowerKeyword = keyword.toLowerCase();
        return allPrompts.filter(p => p.title.toLowerCase().includes(lowerKeyword) ||
     ...
     * HTTP POST请求（辅助方法）
     */
    static async httpPost(endpoint, data) {
        return new Promise((resolve, reject) => {
            // 实现HTTP POST请求
            // 这里需要根据实际API实现
            reject(new Error('Not implemented'));
...
exports.CloudShareManager = CloudShareManager;
// 这里使用模拟的API端点，实际应该替换为真实的API
CloudShareManager.API_BASE_URL = 'https://api.prompthelper.com'; // 示例URL
CloudShareManager.USE_MOCK = true; // 使用模拟数据
//# sourceMappingURL=cloudShare.js.map