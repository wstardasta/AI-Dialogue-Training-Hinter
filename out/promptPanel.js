...
class PromptPanel {
    constructor(_extensionUri, promptManager) {
    ...
    _createPanel() {
        const panel = vscode.window.createWebviewPanel(PromptPanel.viewType, '提示词助手', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async _deletePrompt(promptId) {
        const result = await vscode.window.showWarningMessage('确定要删除这个提示词吗？', '确定', '取消');
        if (result === '确定') {
    ...
    async _toggleFavorite(promptId) {
    ...
    _updateWithPrompts(prompts) {
        if (!this._panel) {
            return;
        }
        this._panel.webview.html = this._getHtmlForWebview(prompts);
    }
    _getHtmlForWebview(prompts) {
        const categories = Array.from(new Set(prompts.map(p => p.category)));
        const categoriesHtml = categories.map(cat => `<option value="${cat}">${cat}</option>`).join('');
...
PromptPanel.viewType = 'promptHelperPanel';
